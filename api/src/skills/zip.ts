...
import { downloadBlobToBuffer } from '../azure/blobClient';
import { SkillDetail } from '../types';

export async function streamSkillZip(detail: SkillDetail, res: Response): Promise<void> {
  ...
  for (const file of detail.files) {
    ...
    const buffer = await downloadBlobToBuffer(blobName);
